...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={cn(
        "relative h-6 w-11 shrink-0 rounded-full transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]/40",
        checked
          ? "bg-[var(--accent)] shadow-[0_0_12px_rgba(59,130,246,0.35)]"
          : "bg-[color-mix(in_srgb,var(--accent)_20%,white)]",
      )}
    >
      <span
        className={cn(
          "absolute top-0.5 size-5 rounded-full bg-white shadow-[0_1px_4px_rgba(30,80,160,0.3)] transition-all",
      ...
      <Card className="mt-6 p-6">
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center justify-between rounded-xl bg-[var(--bg-hover)] px-4 py-3">
            <div>
              <p className="text-sm font-medium text-[var(--ink)]">启用点赞</p>
              <p className="text-xs text-[var(--ink-3)]">关闭后文章页不显示点赞按钮</p>
            </div>
