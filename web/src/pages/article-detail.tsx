...
export default function ArticleDetailPage() {
  ...
  return (
    <article>
      ...
      <Link
        to="/articles"
        className="mb-6 inline-flex items-center gap-1 text-sm text-[var(--ink-3)] hover:text-[var(--ink)] transition-colors"
      >
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-bold leading-snug tracking-tight text-[var(--ink)]">
          {article.title}
        </h1>
        {/* meta 行：等宽 + tabular-nums（数字对齐），数据文字用 --ink-* 蓝灰阶 */}
        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-[13px] tabular-nums text-[var(--ink-2)]">
          {article.author && (
            <span className="inline-flex items-center gap-2">
              ...
              <span className="font-medium text-[var(--ink)]">{article.author.username}</span>
            </span>
          )}
      ...
      {article.cover_image && (
        <img
          ...
          alt="封面"
          className="mb-8 w-full rounded-[var(--radius-card)] border border-[var(--border-card)] object-cover"
        />
      )}
