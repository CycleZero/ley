...
func (m *mockArticleRepo) List(ctx context.Context, query ArticleListQuery) ([]*Article, int64, error) {
	...
	for _, a := range m.articles {
		if query.AuthorID != nil && a.AuthorID != *query.AuthorID {
			continue
		}
		cp := *a
...
func (m *mockArticleRepo) Search(ctx context.Context, keyword string, page, pageSize int) ([]*Article, int64, error) {
	...
	for _, a := range m.articles {
		...
		if !strings.Contains(a.Title, keyword) && !strings.Contains(a.Content, keyword) {
			continue
		}
		cp := *a
		result = append(result, &cp)
...
func (m *mockArticleRepo) InsertLike(ctx context.Context, articleID, userID uint) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.likes[articleID] == nil {
		m.likes[articleID] = make(map[uint]bool)
	}
...
func (m *mockArticleRepo) DeleteLike(ctx context.Context, articleID, userID uint) error {
	...
	defer m.mu.Unlock()
	if m.likes[articleID] != nil {
		delete(m.likes[articleID], userID)
	}
	return nil
}
...
func (m *mockCache) GetObject(ctx context.Context, key string, value any) error {
...
func (m *mockCache) GetOrSet(ctx context.Context, key string, loader func() (any, error), expiration time.Duration) ([]byte, error) {
	return nil, cache.ErrKeyNotFound
}
func (m *mockCache) GetDel(ctx context.Context, key string) ([]byte, error) {
	return nil, cache.ErrKeyNotFound
}
func (m *mockCache) GetObjectDel(ctx context.Context, key string, value any) error {
	return cache.ErrKeyNotFound
}
